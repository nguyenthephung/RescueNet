export const CONFIG = {
  API_GATEWAY: "http://localhost:8888/api/v1",
};

export const API = {
  LOGIN: "/identity/auth/token",
  MY_INFO: "/profile/users/my-profile",
  MY_POST: "/post/my-posts",
  CREATE_POST: "/post/create",
  UPDATE_PROFILE: "/profile/users/my-profile",
  UPDATE_AVATAR: "/profile/users/avatar",
  SEARCH_USER: "/profile/users/search",
  MY_CONVERSATIONS: "/message/conversations/my-conversations",
  CREATE_CONVERSATION: "/message/conversations/create",
  CREATE_MESSAGE: "/message/messages/create",
  GET_CONVERSATION_MESSAGES: "/message/messages",
};
