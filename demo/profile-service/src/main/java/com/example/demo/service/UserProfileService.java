...
import com.example.demo.dto.request.SearchUserRequest;
...
//    FileClient fileClient;
    ...
    public List<ProfileUserResponse> search(SearchUserRequest request) {
        var userId = SecurityContextHolder.getContext().getAuthentication().getName();
        List<ProfileUser> userProfiles = userProfileRepository.findAllByusernameLike(request.getKeyword());
        return userProfiles.stream()
                .filter(userProfile -> !userId.equals(userProfile.getUserId()))
                .map(userProfileMapper::toProfileUserResponse)
                .toList();
    }
    public ProfileUserResponse getByUserId(String userId) {
        ProfileUser userProfile =
                userProfileRepository.findByusername(userId)
